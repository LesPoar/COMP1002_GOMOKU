    ...
    def ColCheck(self,player):
        for col in range(len(self.board[0])):
            for i in range(len(self.board) - 4):
                if self.board[i][col] == self.board[i+2][col] == player and self.board[i+1][col] != player:
                    self.board[i+1][col] = []
                if [self.board[i+j][col] for j in range(5)] == [player]*5:
                    return True
        return False
    def RowCheck(self,player):
        for row in self.board:
            for i in range(len(row) - 4):
                if row[i] == row[i+2] == player and row[i+1] != player:
                    row[i+1] = []
